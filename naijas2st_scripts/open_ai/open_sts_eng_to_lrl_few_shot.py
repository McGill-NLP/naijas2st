...
Variant of ``openai_stt_eng_to_lrl.py`` configured for the Yoruba /
Hausa / Igbo English->LRL direction with N few-shot ``(audio, English,
LRL)`` demonstrations interleaved as user/assistant turns. Writes
predictions to per-language JSONs.
"""
...
def main():
    ...
    Variant configured for Yoruba/Hausa/Igbo (Pidgin handled by a
    sibling script). Workflow per language:

        1. Resolve the few-shot dir for the LRL's English accent and
           the corresponding English test dir.
        2. Load up to ``number_of_few_shot_examples`` examples from
           the few-shot dir into ``(wav_path, english_src, lrl_tgt)``
           triples; skip files missing either side.
        3. Walk ``english_<accent>/<user>/recordings/*.wav``. For each
           wav:
            - Normalise the audio through ``soundfile`` into a temp dir.
            - Build a chat ``messages`` payload introducing the
              few-shot setup, then for each demo append a ``user`` turn
              with the demo audio + ``"Example i"`` and an
              ``assistant`` turn carrying the LRL translation.
            - Append a final user turn with the base64-encoded test
              audio + explicit "translate this English audio into
              ``<language>``" instruction.
            - Call :func:`call_azure` with up to 5 retries (2 s
              back-off); append ``{"file_name", "prediction"}`` and
              rewrite the per-language results JSON after each call.
